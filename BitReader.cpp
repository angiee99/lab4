...
std::string  BitReader::setStream(std::string inputFileName) {
    ...
    std::string str;
    char c;
    // Read characters until the separator is encountered
...
uint32_t BitReader::readCode(int bitsToRead) {
        // take what is left in buffr
    uint32_t code = bitBuffer;    // code - result of the readCode
    ...
    while (bitsToRead != 0 && inputFile.get((char&)(readChar))) {
        ...
        if (bitsToRead >= 8) {
            ...
            code |= static_cast<uint32_t>(readChar & bitMasks[bitsToRead]) << addedBitsCount;
